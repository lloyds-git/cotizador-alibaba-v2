#!/usr/bin/env python3
"""Migracion: agregar columna pzas_caja a productos.
...
def main() -> int:
    ...
    if columna_existe(con, "productos", "pzas_caja"):
        print("La columna pzas_caja ya existe. Nada que hacer.")
        con.close()
        return 0

    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    backup = DB_PATH.with_name(f"productos.pre-pzas_caja-{ts}.db")
    shutil.copy2(DB_PATH, backup)
    print(f"Backup: {backup}")

    try:
        with con:
            con.execute("ALTER TABLE productos ADD COLUMN pzas_caja INTEGER")
    ...
    if not columna_existe(con, "productos", "pzas_caja"):
    ...
    con.close()
    print(f"Columna pzas_caja agregada. Productos en tabla: {n}.")
    return 0
